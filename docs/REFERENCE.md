...
| 10 | `label_ids` vs `label_names` inconsistency? | ✅ **Closed (2026-05-19, Phase 2 E2E)** — `label_ids` expects UUIDs only. The Postman example showing a string name was misleading. Resolver in `api/_lib/resolve.ts` accepts names and looks them up against `/labels` before sending. | Low | — |
| 18 | `create_test_case` required fields | ✅ **Closed (2026-05-19, Phase 2 E2E)** — TMS requires ALL of `status_id`, `priority_id`, `type_id`, `automation_type_id`, `owner_id` on create. Omitting any returns HTTP 400 with `"Invalid ID format: "` (misleading — actually means missing UUID). Tool fills defaults from lookup tables when fields aren't supplied. | Medium | — |
| 19 | `/users` response envelope shape | ✅ **Closed (2026-05-19, Phase 2 E2E)** — Unlike other list endpoints, `/users` returns `{ message, data: [...] }` (data is a DIRECT array, no inner key, no `page_info`). Client's `getList` now handles both envelope shapes. | Low | — |
| 20 | JWT `sub` claim → user_id mapping | ✅ **Closed (2026-05-19, Phase 2 E2E)** — TMS API key JWT `sub` is a separate auth/service identifier, NOT a TMS user UUID. Using it as `owner_id` returns FK constraint violation. There's no `/users/me` endpoint. Tool defaults `owner_id` to the first ACTIVE user from `/users` when not supplied. | Medium | — |
| 21 | Test plan/run dates are epoch SECONDS or MILLISECONDS? | ✅ **Closed (2026-05-20, Phase 4 E2E)** — All dates on test plans + test runs are **epoch MILLISECONDS** (Postman doc said "seconds" but real data shows 13-digit ms timestamps: `1766395575161` = 2025-12-22). REFERENCE §5 line for Test Plans was wrong; corrected in tool descriptions. | Low | — |
| 22 | How to close a test run? | ✅ **Closed (2026-05-20, Phase 4 E2E)** — `PUT /test_runs/{id}` accepts a sparse body. Just send `{"status":"CLOSED"}` — no need to fetch + merge. Server auto-fills `closed_by` with the calling user. Full GET+merge+PUT fails for DYNAMIC runs because `dynamic_selection_excluded_test_case_ids: null` returned by GET triggers validation rejection when echoed back. Same sparse pattern likely works for other PUT endpoints. | Medium | — |
| 23 | DYNAMIC test run filter syntax | ⚠️ **Partial (2026-05-20, Phase 4 E2E)** — Empty `dynamic_selection_filters: []` = include all TCs in project (works). Non-empty filter shape `{field, operator, values}` per Postman doc tried with `{field:"folder_id", operator:"EQUALS", values:[<uuid>]}` but did NOT filter (returned all 455 TCs). Real filter syntax probably differs — recommend probing via TMS web UI network traffic for actual queries. Tool accepts filter array pass-through; user supplies correct syntax. | Low | Pakai STATIC selection_type kalau butuh kontrol presisi list TC. |
| 24 | Test run multipart envelope for result marking | ✅ **Closed (2026-05-20, Phase 4 E2E)** — `PUT /test_runs/{id}/test_cases` uses `multipart/form-data` with single text part `data` (no file uploads needed for basic result marking). The `data` field contains JSON `{"test_run_cases":[...]}`. Each entry needs `test_case_id` (UUID), `test_run_status_id` (UUID from `/test_runs/statuses`), `user_id` (UUID), optional `description`. Status names: Passed, Failed, Blocked, Skipped, UnTested, In Progress. | Medium | — |

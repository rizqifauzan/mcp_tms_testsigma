...
export const listTestCasesInputSchema = {
  ...
  search: z.string().optional().describe("Substring match on test case name"),
...
export async function listTestCases(
  ...
  const res = await client.get<PaginatedResponse<TestCase>>(
  ...
  const md = mdTable(
    ["Human ID", "Name", "Template", "UUID"],
    res.data.map((tc) => [tc.human_id ?? "—", tc.name, tc.template_type ?? "—", tc.id]),
...
export async function getTestCase(
  ...
  const tc = await client.get<TestCase>(
    `/projects/${encodeURIComponent(args.project_id)}/test_cases/${encodeURIComponent(args.test_case_id)}`,
  );

  return hybrid(tc, renderTestCase(tc));
}

function renderTestCase(tc: TestCase): string {
  const header = `**${tc.human_id ?? tc.id} — ${tc.name}**`;
  const meta = `Template: \`${tc.template_type ?? "?"}\` · Folder: \`${tc.folder_id ?? "root"}\``;
